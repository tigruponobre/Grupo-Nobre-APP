...
async function createNewDiscipline(){
    ...
    const newFloor = await document.querySelector('.newFloor').value
    const newStart = await document.querySelector('.newStart').value
    const newEnd = await document.querySelector('.newEnd').value

    const response = await fetch(url + '/.netlify/functions/create_discipline_unifan', {
        method: 'put',
        body: JSON.stringify({
            curso: newCourse,
            turno: newShift,
    ...
    if (data.resposta == "Insert successful."){
        ...
        document.querySelector('.newFloor').value = ''
