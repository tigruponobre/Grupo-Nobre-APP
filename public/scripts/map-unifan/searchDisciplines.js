...
async function searchMapDesktop(curso, turno, dia){
    ...
    //Fetch
    const response = await fetch(url + '/.netlify/functions/search_disciplines_unifan', {
        method: 'post',
        body: JSON.stringify({
            curso: curso,
            turno: turno,
...
async function searchMapMobile(curso, turno, dia){
    ...
    for (let elem of Array.from(Object.keys(information))){
        ...
        let newAndar = information[elem]['ANDAR']
        ...
        <th>ANDAR: <td class="tableData">${newAndar}</td></th>
